...
public class ModeInputPanel extends GridPane {
    ...
    private int getFiles(ListView<String> listView) { // TODO
        ...
        File boardsDirectory = new File("C:\\Users\\jgiso\\Documents\\A2\\boards");  // Assuming your board files are in a "Boards" directory
        File[] files = boardsDirectory.listFiles();

        if (files != null) {
            Arrays.stream(files)
                    .filter(file -> file.isFile() && file.getName().toLowerCase().endsWith(".txt"))
                    .forEach(file -> listView.getItems().add(file.getName()));

            // Find the index of "Starter.txt"
            int starterIndex = listView.getItems().indexOf("Starter.txt");
            return (starterIndex != -1) ? starterIndex : 0;  // Return 0 if "Starter.txt" is not found
        } else {
            // Handle the case where the directory doesn't exist or is not a directory
            return -1;  // You can return -1 or another suitable value as an indicator
        }
    }
    ...
    private void selectBoard(Label selectBoardLabel, ListView<String> boardsList) { // TODO
        String selectedBoardFileName = boardsList.getSelectionModel().getSelectedItem();

        if (selectedBoardFileName != null) {
            File selectedBoardFile = new File("C:\\Users\\jgiso\\Documents\\A2\\boards", selectedBoardFileName);
            board.loadBoard(selectedBoardFile);
            selectBoardLabel.setText("Selected Board: " + selectedBoardFileName);
        } else {
            selectBoardLabel.setText("No board selected");
        }
    }
}
